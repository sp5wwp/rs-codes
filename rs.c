...
/* form new elp(x) */
            for (i=0; i<(rs->n)-(rs->k); i++)    elp[u+1][i] = 0 ;
            for (i=0; i<=l[q]; i++)
              if (elp[q][i]!=-1)
...
/* put elp into index form */
         for (i=0; i<=l[u]; i++)   elp[u][i] = rs->index[elp[u][i]] ;
...
/* form polynomial z(x) */
  ...
  /* evaluate errors at locations given by error location numbers loc[i] */
           ...
           for (i=0; i<rs->n; i++)        /* could return error flag if desired */
               if (inp[i]!=-1)        /* convert inp[] to polynomial form */
                 inp[i] = rs->alpha[inp[i]] ;
               else  inp[i] = 0 ;     /* just output received codeword as is */
       }
